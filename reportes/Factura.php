...
function fact_dev($libelle, $num, $numTimbrado, $fechaInicio, $fechaFin)
{
    $r1 = $this->w - 80;
    $r2 = $r1 + 68;
    $y1 = 7;
    $y2 = $y1 + 28; // Ajusta el espacio para acomodar las tres líneas
    $mid = ($r1 + $r2) / 2;
    ...
    // Agregar número de timbrado
    $this->SetXY($r1 + 1, $y1 + 8);
    $this->SetFont("Arial", "B", $szfont); // Usa la misma fuente que el texto principal
    $this->Cell($r2 - $r1 - 1, 5, "Timbrado: " . $numTimbrado, 0, 0, "C");

    // Agregar fecha de inicio
    $this->SetXY($r1 + 1, $y1 + 14);
    $this->Cell($r2 - $r1 - 1, 5, "Inicio: " . $fechaInicio, 0, 0, "C");

    // Agregar fecha de fin
    $this->SetXY($r1 + 1, $y1 + 20);
    $this->Cell($r2 - $r1 - 1, 5, "Fin: " . $fechaFin, 0, 0, "C");
}
...
// Invoice
// Invoice
function addFacture($numfact, $numTimbrado, $fechaInicio, $fechaFin) {
    ...
    $this->Cell(10, 4, "TIMBRADO", 0, 0, "C");
    $this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 5);
    $this->SetFont("Arial", "", 10);
    $this->Cell(10, 5, $numTimbrado, 0, 0, "C");
    ...
    $this->SetXY($r1 + ($r2 - $r1) / 2 - 5, $y1 + 5);
    $this->SetFont("Arial", "", 10);
    $this->Cell(10, 5, $fechaInicio, 0, 0, "C");
...
function addDate($date)
{
    $r1 = $this->w - 80;
    $r2 = $r1 + 68;
    $y1 = 43; // Ajusta este valor para bajar más hacia abajo
    $mid = $y1 + 5;

    // Dibuja el borde del cuadro
    $this->Rect($r1, $y1, $r2 - $r1, 10);

    $this->SetXY($r1 + 2, $mid);
    $this->SetFont("Arial", "B", 10);
    $this->Cell(15, 5, "Fecha Venta:", 0, 0, "L");
    
    $this->SetXY($r1 + 28, $mid);
    $this->SetFont("Arial", "", 10);
    $this->Cell(40, 5, $date, 0, 0, "L");
}
